...
public class HotDogHouse extends JFrame implements ActionListener
{
    ...
    private void tenderKey(int index)
    {
        switch(index)
        {
            ...
            case 18: {
                try
                {
                    tenderAmount = Double.parseDouble(tenderSB.toString());
                }
    ...
    private void tenderOrder()
    {
        ...
        if(tenderAmount >= gTotal)
        {
            change = 0;
            change = gTotal - tenderAmount;
            if(change == 0)
                change = 0;
            else
                change = -(change);
    ...
    {
        changeStr = String.format("Cash Tendered: $%.2f\nChange: $%.2f", tenderAmount, change);
        changeLbl = new JLabel("<html><p align=\"right\">" 
